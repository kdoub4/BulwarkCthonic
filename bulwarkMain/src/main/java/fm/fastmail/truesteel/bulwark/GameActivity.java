...
import fm.fastmail.truesteel.bulwark.ui.GameTable;
import fm.fastmail.truesteel.bulwark.ui.JoinGameDialog;
...
import fm.fastmail.truesteel.bulwark.util.ThemeSetter;
...
import static fm.fastmail.truesteel.bulwark.ui.Strings.format;
 ...
 *
 * On creation, we had also created a server intent (BulwarkServer), which instantiated VDomServer, which
...
public class GameActivity extends SherlockActivity implements EventHandler {
    ...
    ArrayList<String> getUserPrefs() {
        ...
        if (!prefs.getBoolean("errata_mine", true)) {
    ...
    void startServer() {
        startService(new Intent("fm.fastmail.truesteel.bulwark.SERVER"));
    }
    void stopServer() {
        stopService(new Intent("fm.fastmail.truesteel.bulwark.SERVER"));
    }
    ...
    public static final String BASEDIRFROMEXT = "/Bulwark";
    ...
    final Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (e.t) {
                ...
                case INFORM: // received inform message
                    ...
                    String inform = format(s, Strings.getCardName(e.c));
                    Toast.makeText(top, inform, Toast.LENGTH_LONG).show();
                    break;

                case CARDOBTAINED: // a player got a card
                    setStatus(e.setString(gt.cardObtained(e.i, e.s, e.o.cs)));
                    // ack = true;
                    break;
                ...
                case GETCARD:  //RemotePlayer wants us to choose a card and send an EType.CARD event back
                    gt.selectCard(e.o.sco, e.s, e.i, e.b, e.gameEventType);
                    break;
        ...
        private String buildHintString(GameStatus gs, String s, boolean newTurn) {
            ...
            String attacks;
            if (gs.turnStatus[3] == 1)
                attacks = format("%1$s x", "" + gs.turnStatus[3]);
            else
                attacks = format("%1$s x", "" + gs.turnStatus[3]);

            String might;
            if (gs.turnStatus[4] == 1)
                might = format("%1$s m", "" + gs.turnStatus[4]);
            else
                might = format("%1$s m", "" + gs.turnStatus[4]);

            String melee = "-";
            if (gs.turnStatus[5] == 1)
                melee = "M";

            String range = "-";
            if (gs.turnStatus[6] == 1)
                range = "R";


            String baseStr = format("%1$s, %2$s, %3$s, %4$s, %5$s, %6$s, %7$s", actions, buys, coinsStr, attacks, might, melee, range); //%1$s, %2$s, %3$s.
