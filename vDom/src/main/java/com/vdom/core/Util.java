...
//        }
    ...
    public static int indexOfCardId(int id, ArrayList<Card> cards){
    ...
    private static boolean doSecretChamber(MoveContext context, Game game, Player player, Card responsible, Card reactionCard) {
        ...
        if (found) {
            ...
            if (player.hand.size() > 0) {
                Card[] cards = player.controlPlayer.action_cardsToPutOnDeck(context,Cards.secretChamber , 2 );
                boolean bad = false;
	 ...
	 * Crown cards first, rest unsortet
	 */
